...
watch(focusedIndex, (newIndex) => {
    // debug
    console.log('Inner Focused index changed:', newIndex);
});
...
const updateFocused = (newIndex: number) => {
    ...
    // debug
    console.log('Updating focus to card index:', newIndex, new Error())
...
const handleMouseUp = (event: MouseEvent) => {
    if (!isDragging) return; // 如果没有拖动则不处理
    const deltaX = event.clientX - startX;
    if (Math.abs(deltaX) < CLICK_THRESHOLD) {
        // 如果拖动距离小于阈值，认为是点击
        handleClick(event);
    ...
    if (scrollPart.value) scrollPart.value.style.cursor = 'default'; // 恢复默认光标
...
const handleCardClick = (index: number) => {
    if (!scrollPart.value || index < 0 || index >= scrollPart.value.children.length) return;
